...
func TestReportShouldSuccessfullyReportResults(t *testing.T) {
	t.Run("on checkup success", func(t *testing.T) {
		...
		expectedReportData := map[string]string{
			...
			"status.result.platform":                                  checkupStatus.Results.Platform,
			"status.result.ocpVersion":                                checkupStatus.Results.OCPVersion,
			"status.result.cnvVersion":                                checkupStatus.Results.CNVVersion,
			"status.result.k8sVersion":                                checkupStatus.Results.K8sVersion,
			"status.result.kubevirtVersion":                           checkupStatus.Results.KubeVirtVersion,
